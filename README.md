...
> **The seed is clean.** A first control on the seeded project raises no findings and leaves no document unread. The walkthrough below is the *demo scenario* — invoice 1147 on the wrong section, PO 2291 keyed in kilograms, the steel remainder at the superseded price, the revised BOQ page — which lives in `tests/fixtures/scenario.ts` and is injected on top of the seed by the tests. There is no script yet that stages it in the live database, so running the agent against a freshly seeded project will find nothing.

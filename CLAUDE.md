...
- `src/hadarim/` — the Hadarim product (entries `hadarim.html` → `App.tsx` for the ERP, `report.html` → `ReportApp.tsx` for the report):
  - `data/` — deterministic generator of the seed package (`generate.ts`), document pages (`documents.ts`), types, `bluebook.ts` (the chapters of the Interministerial Specification and the trade terms of each chapter, reference data). Used for seeding and for offline/tests; the live data is in Supabase.
...
- A record with no contract is still checked for its section: `describedSection` reads its own description against each section's terms — the words of the section's name and the terms of the Blue Book chapters it covers (`CHAPTER_TERMS`, reference data, never scenario words) — and points elsewhere only when the wording uses at least two terms of exactly one other section and none of the section the record sits on. Terms belong in `bluebook.ts`, keyed by chapter, never a section id or a supplier.

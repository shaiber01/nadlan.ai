...
- The demo scenario's errors — PO 2291 keyed in kilograms, the steel remainder at the superseded appendix price, the revised BOQ page that drops a line — live in `tests/fixtures/scenario.ts`, not in the seed, and the test files that walk the script install them (`installScenario()`). Put a new error there too. What a control does or does not find is never stated in prose: the checks derive it from the data, and `tests/hadarim.data.test.ts` is what pins it.

...
- Every table is keyed by `project_id`; `HADARIM` is the seeded project. Nothing in `engine/`, `tools/`, `features/` or the agent may hard-code a record, supplier, document, section id, person or amount of the demo scenario — derive it from the package (`pkg` in `engine/package.ts`: sections with `shortHe` and `kind`, people by role, contracts, appendices, BOQ, forecast versions, documents' `facts`) and the live ERP state. Scenario specifics belong in `data/` (the seed) and in the tests. Policy constants that come from the report standard (materiality thresholds) live in `engine/report.ts` and are named as such.

...
- **The seed is clean**: a first control on the seeded project raises no findings and leaves no document unread. The demo's three errors — PO 2291 keyed in kilograms, the steel remainder at the superseded appendix price, the revised BOQ page that drops a line — live in `tests/fixtures/scenario.ts` and are injected on top of the seed by the test files that walk the script (`installScenario()`). Add a new error there, never to the seed, and keep `tests/hadarim.data.test.ts`'s "a first control on the seed raises nothing" passing.

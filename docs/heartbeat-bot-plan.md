...
What remains is not code: the user's Vonage setup (§13, phase 2) for the first real WhatsApp message, and phase 5's later items (routing by the card's people, `ask_person` over the channel, the Word report as a file, delivery status, reminders, the Agent SDK runner).

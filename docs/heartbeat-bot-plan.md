...
create table public.monitor_settings (
  ...
  notify_on_quiet   boolean not null default false,        -- a one-line summary when the pass found nothing to decide (off: logged only)
...
**Quiet passes stay in the log (2026-09-15).** A heartbeat that found nothing to decide no longer sends the one-line summary to the phones: `notify_on_quiet` defaults to off (migration `20260915130000`, the project's row switched off), the relay keeps the reply back and logs it. Also from the first night on the real channel: a send to Vonage that never returned had held the relay and the scheduler since the evening, and a failed settings read would have ended the timer chain — every Vonage call now carries a deadline and a tick whose database read fails retries with a backoff.

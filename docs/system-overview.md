...
2. Each finding is a card with four blocks — הבעיה · המקורות · המשמעות · ההחלטה הנדרשת — plus the people connected to the record (who entered, approved, changed it) and, when the right values are determined by the data, a proposed fix. Every decision option carries `consequenceHe`: what the engine does the moment it is chosen (the ERP write, the task for the named person, the closed or the open finding), with names from the project's people. The agent quotes the card as returned — no interpretation, sources already shown on an earlier card referenced rather than repeated, the people line always — then asks for the decision with Claude Code's question tool (`AskUserQuestion`): clickable options, the recommended fix first, each option's description its consequence, "Other" for free text. The same tool is used for a found quote, who is deciding, and confirmations (reset, save the configuration, finalize). Delegated as a subagent it cannot prompt: it returns the card and its options, the delegating session asks the user and continues the same agent with the answer (the relay protocol in `CLAUDE.md`).

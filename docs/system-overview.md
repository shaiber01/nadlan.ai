...
| חוזי קבלני משנה | Read: scope, inclusions, exclusions, retention, price appendices, documents; the price schedule — the BOQ lines the contract covers with their unit prices, totalled against the contract amount (a lump-sum contract's breakdown) | Edit a line's unit price in place (attributed, with a note; logged as record type `boq_line`) |
| כתב כמויות | Read: the bill of quantities by Blue Book chapter — every line with its quantity, unit, unit price and value (₪ before VAT; the priced lines of a lump-sum contract are that contract's breakdown and sum to it), budget section and contract coverage (the covering contract, or the exclusion clause that leaves it open), chapter totals, filters by chapter, section, coverage and text; click through to the covering contract or the line's document page. A line is addressable by URL (`?screen=boq&line=57.03.040`), which is how the report's coverage sources open it | — |
...
| Attributed ERP writes | `reallocate_invoice`, `correct_purchase_order`, `set_invoice_building`, `create_invoice`, `set_boq_unit_price`, `add_budget_change` |
...
2. Each finding is a card with four blocks — הבעיה · המקורות · המשמעות · ההחלטה הנדרשת — plus the people connected to the record (who entered, approved, changed it) and, when the right values are determined by the data, a proposed fix. Every decision option carries `consequenceHe`: what the engine does the moment it is chosen (the ERP write, the task for the named person, the closed or the open finding), with names from the project's people. The agent quotes the card as returned — no interpretation, sources already shown on an earlier card referenced rather than repeated, the people line always — then asks for the decision with Claude Code's question tool (`AskUserQuestion`): clickable options, the recommended fix first, each option's description its consequence, "Other" for free text. Several findings on one invoice or order fold into one card when every one of them carries a data-determined fix and the fixes agree field by field (`groupByRecord`, a post-pass of `runChecks`, so the control session, the heartbeat, the report's open findings and `run_check` with kind `all` see the same card): kind `record`, id `F-REC-<invoice>` / `F-REC-PO-<order>`, one problem line per member, the union of their sources, one union fix, one question; ungrouped findings keep their ids. Deciding the card decides every member — `apply` writes all the fields through the guarded commands with one §4b correction row per field written, each attributed to the member whose fix named it; `refer` opens one task for the record; `accept` closes every member — and the decision and audit are recorded for the card and for each member. A finding without a fix (a judgment card) or fixes that conflict on a field stay their own cards; the member checks stay individually runnable; a member's id resolves to its card; the heartbeat counts a card as presented through its members and a member through its record's card. The same tool is used for a found quote, who is deciding, and confirmations (reset, save the configuration, finalize). Delegated as a subagent it cannot prompt: it returns the card and its options, the delegating session asks the user and continues the same agent with the answer (the relay protocol in `CLAUDE.md`).
...
§2א (`engine/kpis.ts`, `ReportModel.kpis`) is the report's benchmarking layer: ₪ per gross m² and per unit (the project's `grossSqm` and `units`) for the whole project, per cost group and per section, and the material indices read off the bill of quantities by Blue Book chapter and unit — steel (kg/m²), concrete (m³/m²), formwork, earthworks, blockwork, waterproofing, plaster, flooring, plus steel per m³ of concrete — each with the BOQ quantity, the delivered quantity (only from invoices that carry a quantity, converted into the index unit; otherwise the row says what "done" rests on), the budget / paid / current unit price (appendix in force, signed contract, or estimate — labelled), the cost per m² (the section's EAC when the material is the whole section, else the BOQ value — labelled) and a verdict against the project's reference ranges (`kpi_policy` on the project row; `STANDARD_KPI_POLICY` defaults; `set_project_status` `kpiRanges`). The cost groups (שלד / מעטפת וגמר / מערכות / פיתוח / תקורה / בלתי צפוי) and the material index definitions are reference data in `data/bluebook.ts`. On the page: a KPI strip under the masthead, the §2א tables and two charts, material cards under the §5 analysis of a section measured in quantities; the sections table is interactive (sort by any column, filter to material or soft rows, search, ₪ / ₪ per m² / ₪ per unit views scaled by the engine, a row opens to its forecast lines, rarely-read columns fold away on screen and are always printed and exported); every other table sorts by its columns. Word, Excel and Markdown carry §2א and the material cards.
...
`src/hadarim/engine/` is pure TypeScript over the project package (`pkg`, bound by `package.ts` to whatever project was loaded): `checks.ts` (the checks → findings, with people and proposed fixes; option consequences; the findings of one record folded into one card), `forecast.ts` (the working forecast: recorded, committed, remaining commitment, uncovered by basis, EAC, variance per section; original budget + changes), `commands.ts` (state commands: ERP edits, control run, decisions, report config), `operations.ts` (adjustments, tasks, notes, questions, raised findings, instructed corrections), `report.ts` (the report model), `heartbeat.ts` (what is new since a watermark), `units.ts` (unit families and conversion), `model.ts` (session types). `db/session.ts` loads a `V2State` from the database and saves the diff back; the browser store and the tools use the same functions. Exports: `export/docx.ts`, `export/markdown.ts`, `export/xlsx.ts`.

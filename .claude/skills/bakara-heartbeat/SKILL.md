...
4. **Findings** — `findings` are the checks' findings that touch these records. Present each as a card: הבעיה · המקורות · המשמעות · the recommended fix (`proposedFix`) and the people involved (`people`). With a user present, get the decision through `/bakara-control` — each card's decision asked with `AskUserQuestion` (`run_control` first if the control has not run, so decisions can be recorded; findings from your reading go in with `raise_finding`). With no user (scheduled run, `claude -p`) ask nothing: present them in the summary and leave them open — never apply a fix nobody approved.

...
## Replaced documents
A document can be replaced by a newer upload (from the record's card or the folder's list in the ERP — seed pages included — or `add_document` with `replacesDocumentId`). The replaced document keeps its row, marked (`supersededBy`); `search_documents` hides it unless `includeReplaced`, it is never pending, and the checks (a record's document, the BOQ page and the quote for a BOQ line) and the record's card use the current document only. When a heartbeat brings a pending document that replaced another, read the new one and record its facts as usual — do not re-read the replaced one.

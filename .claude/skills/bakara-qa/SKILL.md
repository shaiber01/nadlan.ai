...
| Cost per m² / per unit, how the project compares — ₪ למ״ר ברוטו (total and by cost group: שלד, מעטפת וגמר, מערכות, פיתוח, תקורה), ברזל ק״ג/מ״ר, בטון מ״ק/מ״ר, יחס ברזל לבטון, formwork, plaster, flooring per m², delivered quantities and the budget / paid / current unit prices, the project's reference ranges | `build_report` (`summary.kpis`: `total`, `groups`, `materials`, `derived`; `rangeStatus` says בטווח / מעל / מתחת / ללא טווח). The ranges are the project's KPI policy (`get_project.kpiPolicy`); an index outside its range is a question to check, not a finding |

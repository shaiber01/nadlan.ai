...
export function reportToWorkbook(report: ReportModel, tab: "full" | "ceo" = "full"): Uint8Array {
  ...
  sheets.push(sheet("סעיפים", ["סעיף", "שם", "תקציב מאושר", "נרשם", "התחייבויות", "יתרת התחייבות", "יתרה לא מכוסה", "תחזית לגמר", "סטייה ₪", "סטייה %", "תחזית קודמת", "שינוי", "בסיס %"], rows.map((r) => [r.nameHe === "סה״כ" ? "" : r.sectionId, r.nameHe, r.budget, r.recorded, r.committed, r.remainingCommitment, r.uncovered, r.eac, r.variance, Math.round(r.variancePct * 100) / 100, r.previousEac, r.change, r.basisPct]), [8, 40, 16, 16, 16, 16, 16, 16, 14, 10, 16, 14, 10]));

...
/**
 * A purchase order's amount is never its own field — it is `lineValue()`, the quantity converted into
 * the priced unit and multiplied by the unit price. Correcting quantity, unit or price therefore always
 * recomputes and saves the amount to match, even when that no longer matches what was originally ordered
 * or approved; the source of truth is the line's own arithmetic.
 */

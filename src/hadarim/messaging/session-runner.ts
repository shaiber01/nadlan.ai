...
export function runnerOptionsFromEnv(env: NodeJS.ProcessEnv = process.env): RunnerOptions {
  const budget = env.MONITOR_MAX_BUDGET_USD;
  return {
    bin: env.CLAUDE_BIN || "claude",
    agent: env.MONITOR_AGENT || DEFAULT_AGENT,
    model: env.MONITOR_MODEL || undefined,
...
export function runnerArgs(input: TurnInput, opts: RunnerOptions = {}): string[] {
  ...
  const args = ["-p", "--agent", opts.agent ?? DEFAULT_AGENT, "--output-format", "json", "--permission-mode", "manual", "--permission-prompts", "none", "--allowedTools", allowed.join(","), "--disallowedTools", disallowed.join(",")];
  if (input.sessionId) {
  ...
  // the prompt is the last positional argument; a leading space keeps a text that starts with "-" positional
  args.push(input.text.startsWith("-") ? ` ${input.text}` : input.text);
  return args;
}

...
export function channelContextHe(participants: Participant[], channel: ChannelId, opts: ContextOptions = {}): string {
  ...
  return [
    ...
    "כשמישהו שולח קובץ (צילום חשבונית, הצעת מחיר, PDF), ההודעה נפתחת ב-[קובץ התקבל: נתיב מקומי]: הוסף אותו לתיקיית המסמכים של הפרויקט (add_document עם הנתיב, סוג משוער והכותרת שהשולח כתב — בלי לשאול קודם), קרא אותו (get_document ו-Read), סווג ורשום את עובדותיו (classify_document, set_document_facts), ואם הוא שייך לחשבון, הזמנה או חוזה — הרץ עליהם את הבדיקות (run_check); אז ספר בקצרה מה קראת ומה מצאת, בלי לכתוב נתיבים.",
